...
import logging
import config
from fl_hist import FL_HIST_ALL_STAGE
...
TRAINING_EVALUATION_RATIO = 2
RUNS = 2  # default value = 5
EPISODES_PER_RUN = 200
# STEPS_PER_EPISODE = 200
MAX_EP_STEP = 500  # 每个episode的最大steps
...
def main():
    ...
    env = FL_HIST_ALL_STAGE(fl_config)
    ...
    RL_path = './results/rl/interval_all_RL_{}_{}client1.npz'.format(fl_config.dataset_name, fl_config.clients.total)
    reward_path ='./results/rl/interval_all_RL_reward_{}_{}client1.npz'.format(fl_config.dataset_name, fl_config.clients.total)
    actor_path = './results/rl/interval_all_actor_model_{}_{}client1'.format(fl_config.dataset_name, fl_config.clients.total)
    for run in range(RUNS):
        ...
        for episode_number in range(EPISODES_PER_RUN):
            ...
            evaluation_episode = episode_number % fl_config.fl.stages % TRAINING_EVALUATION_RATIO == 0
            episode_reward = 0
            ...
            for ep in range(MAX_EP_STEP):
                ...
                if ep % fl_config.fl.stages == 0:
                    ...
                    env.get_comm_one_stage(action)

                val_action = copy.copy(action)
                env.synchronize_hists(ep, val_action)

                # RL选action，即selected clients以及cluster local iteration
                # 这里返回值，修改成action_list的index，
                # action = agent.get_nex t_action(state, evaluation_episode=evaluation_episode)
                acc, done = env.step(ep)  # one round FL training
                ...
                if (ep + 1) % fl_config.fl.stages == 0:  # 为一个FL stage结束
                    agent.train_on_transition(state, action, env.next_state, env.reward, done)
                    print("transition:", state, action, env.next_state, env.reward)
                    state = copy.deepcopy(env.next_state)
                    episode_reward += env.reward
                    episode_reward_list.append(episode_reward)
                    plot_x = np.append(plot_x, MAX_EP_STEP * run + ep)
                    plot_acc = np.append(plot_acc, acc)
                    plot_reward = np.append(plot_reward, env.reward)
