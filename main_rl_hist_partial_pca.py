...
import logging
import config
from fl_hist import FL_HIST_PARTIAL
...
class SACAgent:
    ...
    def get_action_nondeterministically(self, state):
        ...
        client_actions = []
        edges_action_probabilities = [0] * self.clusters
        for i in range(self.clusters):
            edges_action_probabilities[i] = action_probabilities[i * self.num_clients:(i + 1) * self.num_clients]
        ...
        for i in range(self.clusters):
            client_choice = list(np.random.choice(range(0, self.num_clients),
                                                  size=self.per_round,
                                                  p=edges_action_probabilities[i], replace=False))
        # client_choice = [client_choice[i] + c*self.num_clients for i in range(len(client_choice))]
            discrete_action = discrete_action + [client_index + i*self.num_clients for client_index in client_choice]
    ...
    def get_action_deterministically(self, state):
        ...
        for i in range(self.clusters):
            for j in range(self.per_round):
                index = np.argmax(edges_action_probabilities[i])
                edges_action_probabilities[i][index] = 0
                discrete_action.append(index+i * self.num_clients)
...
class ReplayBuffer:
    ...
    def get_transition_type_str(self, environment):
        ...
        action_dim = environment.config.clients.per_round*environment.config.clusters
...
def main():
    ...
    env = FL_HIST_PARTIAL(fl_config)
    # state，由global model和local model的PCA降维表示
    env.state_space = (fl_config.clients.total + 1) * \
                      (fl_config.data.hidden_channels + env.labels.max().item() + 1)
    ...
    RL_path = './results/rl/pca_cluster_wise_CS_RL_{}_{}client1.npz'.format(fl_config.dataset_name, fl_config.clients.total)
    reward_path ='./results/rl/pca_cluster_wise_CS_RL_reward_{}_{}client1.npz'.format(fl_config.dataset_name, fl_config.clients.total)
    actor_path = './results/rl/pca_cluster_wise_CS_actor_model_{}_{}client1'.format(fl_config.dataset_name, fl_config.clients.total)
    for run in range(RUNS):
        ...
        for episode_number in range(EPISODES_PER_RUN):
            ...
            state = env.get_initiate_state_pca()  # state初始化
            done = False
            ...
            for ep in range(MAX_EP_STEP):
                ...
                print("action:", action)
                next_state, acc, reward, done = env.step_pca(ep, action)  # one round FL training
                if ep == MAX_EP_STEP - 1:  # 最后一个episode
                    reward = reward - (fl_config.data.target_accuracy - acc) * 10
