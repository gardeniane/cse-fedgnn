...
class Actor(torch.nn.Module):
    ...
    def __init__(self, input_dimension, output_dimension, output_activation,
                 num_clients, num_clusters, period_max):
        ...
        self.layer_out1 = torch.nn.Linear(in_features=128, out_features=period_max)  # 输出之一，client number
        # self.layer_out2 = torch.nn.Linear(in_features=128, out_features=period_max)  # 输出之二，local iteration number
        ...
        self.num = num_clusters  # cluster number
    ...
    def forward(self, inpt):
        ...
        for i in range(self.num):  # 为每个cluster决定通信period
...
class SACAgent:
    ...
    def __init__(self, environment):
        ...
        self.num_edges = environment.config.clusters  # cluster number
    ...
    def get_action_nondeterministically(self, state):
        ...
        action_edge_num = self.period_max
        client_actions = []
        ...
        for i in range(self.num_edges):
            ...
            discrete_action.append(np.random.choice(range(0, self.period_max),
                                                    p=edges_action_probabilities[i][0: self.period_max]))
    ...
    def get_action_deterministically(self, state):
        ...
        for i in range(self.num_edges):
            discrete_action.append(np.argmax(edges_action_probabilities[i][0: self.period_max]))
...
def main():
    ...
    # 算法2.1：所有clusters设置统一的通信period
    # env = FL_HIST_SYN(fl_config)
    # env.fgl_embed_system_level(10)

    env = FL_HIST_ASYN(fl_config)

    # 算法2.2：clusters设置不同的通信period, 从固定的period list中随机取
    # env.fgl_embed_cluster_level([1,1,1,10,10,10,40,40,40,40])

    # 算法2.3：用RL算法设置cluster的通信period
    ...
    RL_path = './results/rl/hist_RL_{}_{}client1.npz'.format(fl_config.dataset_name, fl_config.clients.total)
    reward_path ='./results/rl/hist_RL_reward_{}_{}client1.npz'.format(fl_config.dataset_name, fl_config.clients.total)
    actor_path = './results/rl/hist_actor_model_{}_{}client1'.format(fl_config.dataset_name, fl_config.clients.total)
    for run in range(RUNS):
        ...
        for episode_number in range(EPISODES_PER_RUN):
            ...
            for ep in range(MAX_EP_STEP):
                ...
                action = [0] * fl_config.clients.per_round + action
                next_state, acc, reward, done = env.step(ep, action)  # one round FL training
                action = action[fl_config.clients.per_round:]
