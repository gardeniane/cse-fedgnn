import torch
import torch.nn.functional as F
from sklearn import metrics

def accuracy(output, labels):
    preds = output.max(1)[1].type_as(labels)
    correct = preds.eq(labels).double()
    correct = correct.sum()
    if correct / len(labels) > 1:
        print('111')
    return correct / len(labels)

def test(model, features, adj, labels, idx_test):
    model.eval()
    # output = model(features, adj, adj)
    output = model(features, adj)
    pred_labels=torch.argmax(output, axis=1)
    loss_test = F.nll_loss(output[idx_test], labels[idx_test])
    acc_test = accuracy(output[idx_test], labels[idx_test])
    
    #acc_test = metrics.accuracy_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy())
    #f1_test=metrics.f1_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy(),average='weighted')
    #auc_test=metrics.roc_auc_score(one_hot(labels[idx_test].cpu().detach().numpy()), output[idx_test].cpu().detach().numpy(),multi_class='ovr',average='weighted')
    
    return loss_test.item(), acc_test.item()#, f1_test, auc_test

def test_local(model, features, adj, labels, communicate_index, local_idx_test):
    model.eval()
    # output = model(features, adj, adj)
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index])
    pred_labels = torch.argmax(output, axis=1)
    # 用于记录哪些nodes是当前client的，对应index的值为True
    # node_mask = features.new_empty(features.shape[0], dtype=torch.bool)
    # node_mask.fill_(False)
    # node_mask[client_nodes] = True
    # local_idx_test = node_mask & idx_test
    in_com_test_data_indexes = torch.searchsorted(communicate_index, local_idx_test).clone()
    loss_test = F.nll_loss(output[in_com_test_data_indexes], labels[in_com_test_data_indexes])
    acc_test = accuracy(output[in_com_test_data_indexes], labels[in_com_test_data_indexes])
    return loss_test.item(), acc_test.item()  # , f1_test, auc_test


def test_local_hist(model, features, adj, labels, communicate_index, local_idx_test):
    model.eval()
    # output = model(features, adj, adj)
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index], model, None, None, None, None, None, 0, 0)
    pred_labels = torch.argmax(output, axis=1)
    # 用于记录哪些nodes是当前client的，对应index的值为True
    # node_mask = features.new_empty(features.shape[0], dtype=torch.bool)
    # node_mask.fill_(False)
    # node_mask[client_nodes] = True
    # local_idx_test = node_mask & idx_test
    in_com_test_data_indexes = torch.searchsorted(communicate_index, local_idx_test).clone()
    loss_test = F.nll_loss(output[in_com_test_data_indexes], labels[in_com_test_data_indexes])
    acc_test = accuracy(output[in_com_test_data_indexes], labels[in_com_test_data_indexes])
    return loss_test.item(), acc_test.item()  # , f1_test, auc_test


def cluster_train_loss(model, features, adj, labels, communicate_index, local_idx_test):
    model.eval()
    # output = model(features, adj, adj)
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index], model, None, None, None, None, None, 0, 0)
    pred_labels = torch.argmax(output, axis=1)
    # 用于记录哪些nodes是当前client的，对应index的值为True
    # node_mask = features.new_empty(features.shape[0], dtype=torch.bool)
    # node_mask.fill_(False)
    # node_mask[client_nodes] = True
    # local_idx_test = node_mask & idx_test
    in_com_test_data_indexes = torch.searchsorted(communicate_index, local_idx_test).clone()
    loss_test = F.nll_loss(output[in_com_test_data_indexes], labels[in_com_test_data_indexes])
    acc_test = accuracy(output[in_com_test_data_indexes], labels[in_com_test_data_indexes])
    return loss_test.item(), acc_test.item()  # , f1_test, auc_test


def test_hist(model, features, adj, labels, idx_test):
    model.eval()
    # output = model(features, adj, adj)
    output = model(features, adj, model, None, None, None, None, None)
    pred_labels = torch.argmax(output, axis=1)
    loss_test = F.nll_loss(output[idx_test], labels[idx_test])
    acc_test = accuracy(output[idx_test], labels[idx_test])

    # acc_test = metrics.accuracy_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy())
    # f1_test=metrics.f1_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy(),average='weighted')
    # auc_test=metrics.roc_auc_score(one_hot(labels[idx_test].cpu().detach().numpy()), output[idx_test].cpu().detach().numpy(),multi_class='ovr',average='weighted')

    return loss_test.item(), acc_test.item()  # , f1_test, auc_test

def test_hist1(model, features, adj, labels, idx_test):
    model.eval()
    # output = model(features, adj, adj)
    output = model(features, adj, model, None, None, None, None, None, 0, 0)
    pred_labels = torch.argmax(output, axis=1)
    loss_test = F.nll_loss(output[idx_test], labels[idx_test])
    acc_test = accuracy(output[idx_test], labels[idx_test])

    # acc_test = metrics.accuracy_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy())
    # f1_test=metrics.f1_score(labels[idx_test].cpu().detach().numpy(), pred_labels[idx_test].cpu().detach().numpy(),average='weighted')
    # auc_test=metrics.roc_auc_score(one_hot(labels[idx_test].cpu().detach().numpy()), output[idx_test].cpu().detach().numpy(),multi_class='ovr',average='weighted')

    return loss_test.item(), acc_test.item()  # , f1_test, auc_test


def test_cluster(model, data, labels):
    model.eval()
    out = model(data.x, data.edge_index)
    pred = out.argmax(dim=1)  # Use the class with highest probability.
    loss_test = F.nll_loss(out[data.test_mask], labels[data.test_mask])

    accs = []
    for mask in [data.train_mask, data.val_mask, data.test_mask]:
        correct = pred[mask] == data.y[mask]  # Check against ground-truth labels.
        accs.append(int(correct.sum()) / int(mask.sum()))  # Derive ratio of correct predictions.
    return loss_test.item(), accs


def train(epoch, model, optimizer, features, adj, labels, idx_train):  #Centralized

    model.train()
    optimizer.zero_grad()
    output = model(features, adj)
    loss_train = F.nll_loss(output[idx_train], labels[idx_train])
    acc_train = accuracy(output[idx_train], labels[idx_train])
    #acc_train = metrics.accuracy_score(pred_labels[idx_train].cpu().detach().numpy(), labels[idx_train].cpu().detach().numpy())
    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    
    #acc_val = metrics.accuracy_score(pred_labels[idx_val].cpu().detach().numpy(),labels[idx_val].cpu().detach().numpy())
    #print("epoch", epoch, 
    #      "train", loss_train.item(), acc_train.item())
    return loss_train.item(), acc_train.item()


# Centralized clusterGCN
def train_cluster(epoch, model, optimizer, features, adj,
                  labels, idx_train, in_com_train_data_index, communicate_index):
    model.train()
    optimizer.zero_grad()
    # print(features.shape)

    # adj[communicate_index][:,communicate_index]：取出row,column为communicate_index的子矩阵
    output = model(features[communicate_index], adj[communicate_index][:, communicate_index])
    # print('in_com_train_data_index', in_com_train_data_index.shape[0],'communicate_index:',communicate_index.shape[0],'output:',output.shape[0])
    loss_train = F.nll_loss(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    acc_train = accuracy(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    # print(loss_train,acc_train)
    return loss_train.item(), acc_train.item()

    # model.train()
    # loss_train = 0
    # acc_train= []
    # for sub_data in train_loader:  # Iterate over each mini-batch.
    #     out = model(sub_data.x, sub_data.edge_index)  # Perform a single forward pass.
    #     # Compute the loss solely based on the training nodes.
    #     loss = F.nll_loss(out[sub_data.train_mask], sub_data.y[sub_data.train_mask])
    #     loss.backward()  # Derive gradients.
    #     optimizer.step()  # Update parameters based on gradients.
    #     optimizer.zero_grad()  # Clear gradients.
    #     acc_train.append(accuracy(out[sub_data.x[sub_data.train_mask]],
    #                               sub_data.y[sub_data.train_mask]))
    # return loss_train.item(), acc_train


def Block_matrix_train(epoch, model, optimizer, features, adj, labels, split_data_index, idx_train):
    model.train()
    optimizer.zero_grad()

    output = model(features[split_data_index], adj[split_data_index][:, split_data_index]) #adj, keep block matrix
    
    loss_train = F.nll_loss(output[idx_train], labels[split_data_index][idx_train])
    
    
    acc_train = accuracy(output[idx_train], labels[split_data_index][idx_train])
    

    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    
    #print("epoch", epoch, 
    #      "train", loss_train.item(), acc_train.item())
    return loss_train.item(), acc_train.item()


def Block_matrix_train_batch(epoch, model, optimizer, features, adj, labels, split_data_index_list, idx_train_list ):
    model.train()
    optimizer.zero_grad()
    
    loss_train = None
    count = 0 
    for i in range(len(split_data_index_list)):
        split_data_index = split_data_index_list[i]
        idx_train = idx_train_list[i]
        output = model(features[split_data_index], adj[split_data_index][:, split_data_index]) #adj, keep block matrix
        if loss_train == None:
            loss_train = len(idx_train) * F.nll_loss(output[idx_train], labels[split_data_index][idx_train])
        else:
            loss_train += len(idx_train) * F.nll_loss(output[idx_train], labels[split_data_index][idx_train])
        acc_train = accuracy(output[idx_train], labels[split_data_index][idx_train])
        
        count += len(idx_train)
       
    
    loss_train /= count
    
    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    
    #print("epoch", epoch, 
    #      "train", loss_train.item(), acc_train.item())
    return loss_train.item(), acc_train.item()


# communicate_indexes：某个client的training subgraph的nodes集合
# in_com_train_data_indexes：某个client的local nodes在communicate_indexes中的index
def Lhop_Block_matrix_train(epoch, model, optimizer, features, adj, labels, communicate_index, in_com_train_data_index):
    model.train()
    optimizer.zero_grad()

    # adj[communicate_index][:,communicate_index]：取出row,column为communicate_index的子矩阵
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index])
    # 只有in_com_train_data_index的nodes参与梯度下降
    loss_train = F.nll_loss(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    acc_train = accuracy(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    #print(loss_train,acc_train)
    return loss_train.item(), acc_train.item()


def train_histories(epoch, model, optimizer, features, adj,labels, communicate_index,
                    in_com_train_data_index, client_adj_t_partial, in_com_train_nei_indexes):
    model.train()
    optimizer.zero_grad()

    # adj[communicate_index][:,communicate_index]：取出row,column为communicate_index的子矩阵
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index],
                   client_adj_t_partial, in_com_train_nei_indexes)
    # 只有in_com_train_data_index的nodes参与梯度下降
    loss_train = F.nll_loss(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    acc_train = accuracy(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    # print(loss_train,acc_train)
    return loss_train.item(), acc_train.item()


def train_histories_new(epoch, model, optimizer, features, adj,labels, communicate_index,
                    in_com_train_data_index, client_adj_t_partial, in_com_train_nei_indexes,
                        in_data_nei_indexes, split_data_indexes,
                        in_com_train_local_node_index, global_model):
    model.train()
    optimizer.zero_grad()

    # adj[communicate_index][:,communicate_index]：取出row,column为communicate_index的子矩阵
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index], global_model,
                   client_adj_t_partial, in_com_train_nei_indexes, in_data_nei_indexes,
                   split_data_indexes, in_com_train_local_node_index)
    # 只有in_com_train_data_index的nodes参与梯度下降
    loss_train = F.nll_loss(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    acc_train = accuracy(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    # print(loss_train,acc_train)
    return loss_train.item(), acc_train.item()

def train_histories_new1(epoch, model, optimizer, features, adj,labels, communicate_index,
                    in_com_train_data_index, client_adj_t_partial, in_com_train_nei_indexes,
                        in_data_nei_indexes, split_data_indexes,
                        in_com_train_local_node_index, global_model, period):
    model.train()
    optimizer.zero_grad()

    # adj[communicate_index][:,communicate_index]：取出row,column为communicate_index的子矩阵
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index], global_model,
                   client_adj_t_partial, in_com_train_nei_indexes, in_data_nei_indexes,
                   split_data_indexes, in_com_train_local_node_index, epoch, period)
    # 只有in_com_train_data_index的nodes参与梯度下降
    loss_train = F.nll_loss(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    acc_train = accuracy(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])

    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    # print(loss_train,acc_train)
    return loss_train.item(), acc_train.item()


# 同上面的Lhop_Block_matrix_train一模一样，区别在于in_com_train_data_index的值不一样
def FedSage_train(i, epoch, model, optimizer, features, adj, labels, communicate_index, in_com_train_data_index):
    model.train()
    optimizer.zero_grad()
    #print(features.shape)
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index])
    loss_train = F.nll_loss(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])
    acc_train = accuracy(output[in_com_train_data_index], labels[communicate_index][in_com_train_data_index])
    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    #print(loss_train,acc_train)
    return loss_train.item(), acc_train.item()


def Communicate_train(epoch, model, optimizer, features, adj, labels, communicate_index, split_data_indexes, in_split_train_data_index):
    model.train()
    optimizer.zero_grad()
    #print(features.shape)
    
    output = model(features, adj, [communicate_index, split_data_indexes])

    loss_train = F.nll_loss(output[in_split_train_data_index], labels[split_data_indexes][in_split_train_data_index])
    
    
    acc_train = accuracy(output[in_split_train_data_index], labels[split_data_indexes][in_split_train_data_index])
    '''
    output = model(features[communicate_index], adj[communicate_index][:,communicate_index])
    
    
    loss_train = F.nll_loss(output[in_com_split_data_index][in_split_train_data_index], labels[communicate_index][in_com_split_data_index][in_split_train_data_index])
    acc_train = accuracy(output[in_com_split_data_index][in_split_train_data_index], labels[communicate_index][in_com_split_data_index][in_split_train_data_index])
    '''

    loss_train.backward()
    optimizer.step()
    optimizer.zero_grad()
    #print(loss_train,acc_train)
    return loss_train.item(), acc_train.item()


def ADMM_communication_train(epoch, model, optimizer, features, adj, labels, in_com_train_index, global_state_dict, alpha, lamb):
    model.train()
    optimizer.zero_grad()
    #print(features.shape)
    output = model(features, adj)

    output = output[in_com_train_index]
    loss_train = F.nll_loss(output, labels)
    #for param in model.parameters():
    #reg_loss += l1_crit(param)
    local_parameters=[]
    for i in model.parameters():
        local_parameters.append(i)
    l_index=0
    for key in global_state_dict:
        loss_train += lamb * torch.linalg.norm(local_parameters[l_index]-global_state_dict[key])**2
        loss_train += alpha[key].reshape(-1).dot((local_parameters[l_index]-global_state_dict[key]).reshape(-1)) #####change to parameter
        l_index+=1
    pred_labels=torch.argmax(output,axis=1)
    acc_train = metrics.accuracy_score(pred_labels.cpu().detach().numpy(),labels.cpu().detach().numpy())
    

    loss_train.backward()
    optimizer.step()
    #print(loss_train,acc_train)
    return acc_train, float(loss_train)